...
export const QUADRANTS = [
  {
    key: "UL",
    label: "UL",
    fdis: [18, 17, 16, 15, 14, 13, 12, 11],
    isUpper: true,
  },
  {
    key: "UR",
    label: "UR",
    fdis: [21, 22, 23, 24, 25, 26, 27, 28],
    isUpper: true,
  },
  {
    key: "LR",
    label: "LR",
    fdis: [48, 47, 46, 45, 44, 43, 42, 41],
    isUpper: false,
  },
  {
    key: "LL",
    label: "LL",
    fdis: [31, 32, 33, 34, 35, 36, 37, 38],
    isUpper: false,
  },
];
...
export function getSectionLabel(toothType, viewType, index, fdi) {
  let base =
    ANATOMY_LABELS[toothType]?.[viewType]?.[index] ?? `Section ${index + 1}`;
  if (base === "Tooth") {
    return `Tooth ${fdi}`;
  }
  // For lower teeth, replace "Palatal" with "Lingual"
  if (!isUpperTooth(fdi) && typeof base === "string") {
    base = base.replace(/Palatal/g, "Lingual");
  }
  return base;
}
...
export function isBuccalLabel(label) {
  const l = label.toLowerCase();
  return (
    (l.includes("buccal") || l.includes("facial")) && !l.startsWith("tooth ")
  );
}
...
export function isPalatalLabel(label) {
  const l = label.toLowerCase();
  return (
    (l.includes("palatal") || l.includes("lingual")) && !l.startsWith("tooth ")
  );
}
...
export function getToothImageSrc(fdi, viewType, useImplant = false) {
  ...
  if (viewType === "normal") {
    return useImplant
      ? `/normal-implant-root/${fdi}.png`
      : `/normal-img/${fdi}.png`;
  }
  if (viewType === "normalBack") {
    return useImplant
      ? `/normal-back-view-implant-root/${fdi}.png`
      : `/normal-back-view-img/${fdi}.png`;
  }
