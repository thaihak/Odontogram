...
const LEGEND_ITEMS = [
  ...
  { label: 'Decay', style: { background: 'rgba(229,115,115,0.6)', borderColor: '#e53935' } },
  { label: 'Fracture', className: 'legend-pattern' },
  { label: 'Wear', className: 'legend-color pattern-wear-bg' },
  { label: 'To be Extracted', style: { background: 'rgba(239,68,68,0.1)', border: '1px dashed #ef4444' } },
  { label: 'Missing', style: { background: 'rgba(255,255,255,0.85)', border: '1px dashed #94a3b8' } },
  { label: 'Implant', style: { background: 'transparent', border: '2px dashed #64748b' } },
  { label: 'Braces', style: { background: '#f8fafc', border: '2px solid #cbd5e1' } },
  { label: 'Veneer', style: { background: '#e0f2fe', borderColor: '#0284c7' } },
  { label: 'Pontic', style: { background: '#1e293b', borderColor: '#0f172a' } },
  { label: 'Endo Tests', style: { background: 'rgba(156,39,176,0.5)', borderColor: '#7e22ce' } },
  { label: 'Tooth Eruption', style: { background: 'rgba(34,197,94,0.4)', borderColor: '#16a34a' } },
  ...
  return (
    <div id="mainView">
      <div className="container">
        ...
        <div className={`odontogram${state.ui.sidebarOpen ? ' focus-mode' : ''}`}>
          <div className={`arch${viewMode === 'lower' ? ' hidden' : ''}`}>
            <div className="arch-label">UPPER</div>
            <div className="arch-content" ref={upperRef} style={{ position: 'relative' }}>
          ...
          <div className={`arch${viewMode === 'upper' ? ' hidden' : ''}`}>
            ...
            <div className="arch-content" ref={lowerRef} style={{ position: 'relative' }}>
        ...
        <div className="legend">
          ...
          {LEGEND_ITEMS.map(item => (
            <div key={item.label} className="legend-item">
              <span className={item.className ?? 'legend-color'} style={item.style} />
              <span>{item.label}</span>
            </div>
          ))}
          <br /><br />
          <small>
            <strong>FDI Numbering System:</strong><br />
            Upper Right (Screen Left): 18–11 | Upper Left (Screen Right): 21–28<br />
            Lower Right (Screen Left): 48–41 | Lower Left (Screen Right): 31–38<br />
            <strong>Tip:</strong> <strong>Click a Tooth</strong> to focus it and open the side tool
            palette. Select a condition, then click tooth sections to apply it.{' '}
            <strong>Crown fractures</strong> map to the Top View.{' '}
            <strong>Root fractures</strong> map to the root.
          </small>
        </div>
      ...
      <ul id="actionLogList" className="action-log-list">
        {logs.map(log => (
          <li key={log.id}>
            ...
            Tooth <strong>{log.toothFDI}</strong>:{' '}
            <span style={{ color: '#222', fontWeight: 500 }}>{log.sectionName}</span> marked as{' '}
            <span className={`status-badge ${log.badgeClass}`}>{log.label}</span>
            {log.extraNote ? <span style={{ fontSize: 10.5, fontWeight: 'normal', marginTop: 4, color: '#333', lineHeight: 1.3, display: 'block' }}>{log.extraNote}</span> : null}
          </li>
        ))}
      </ul>
    </div>
