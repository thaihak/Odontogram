...
export default function Tooth({ fdi, isUpper, forceNormalView }) {
  const {
    state,
    focusTooth,
    openEditor,
  ...
  const handleViewDblClick = useCallback(
    (viewType, e) => {
      e.stopPropagation();
      e.preventDefault();
      if (!isFocused) return;
  ...
  // View order based on view mode:
  // All view: Upper (front, top), Lower (top, front) - no back
  // Regular views:
  //   Upper: front, top, back
  //   Lower: back, top, front
  const viewOrder = forceNormalView
    ? isUpper
      ? ["normal", "top"]
      : ["top", "normal"] // all view
    : isUpper
      ? ["normal", "top", "normalBack"] // upper regular view - front, top, back
      : ["normalBack", "top", "normal"]; // lower regular view - back, top, front
  ...
  return (
    <div className={classes} data-fdi={fdi} onClick={handleToothClick}>
      {!forceNormalView && (
        <div className="tooth-number-wrapper">
          ...
          <div className="probing-indicators">
            <div
              className={`probe-dot plaque${probing.plaque ? " active" : ""}`}
              title="Plaque"
            />
            <div
              className={`probe-dot bleeding${probing.bleeding ? " active" : ""}`}
              title="Bleeding"
            />
            <div
              className={`probe-dot pus${probing.pus ? " active" : ""}`}
              title="Pus"
            />
            <div
              className={`probe-dot tartar${probing.tartar ? " active" : ""}`}
              title="Tartar"
            />
          </div>
        </div>
      ...
      <div className="tooth-views">
        {viewOrder.map((viewType, idx) => (
          <div key={viewType}>
            {forceNormalView && !isUpper && idx === 0 && (
              <div
            ...
            />
            {forceNormalView && isUpper && viewType === "top" && (
              <div
                className="tooth-number"
                style={{
                  textAlign: "center",
